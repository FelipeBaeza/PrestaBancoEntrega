...
import { 
  ...
  Receipt, Assessment, Home, History, AccountBalance, 
  Work, Description
...
const FIELD_HELP = {
  ...
  interestRate: "Tasa de interés anual (entre 4% y 6%)",
  term: "Plazo en años (entre 1 y 20 años)",
};

const DOCUMENT_REQUIREMENTS = {
  proofIncome: 'Comprobante de Ingresos',
  appraisalCertificate: 'Certificado de Avalúo',
  propertyWriting: 'Escritura de la Propiedad',
...
const DOCUMENT_ICONS = {
  proofIncome: Receipt,         // Icono de recibo para comprobante de ingresos
  appraisalCertificate: Assessment,  // Icono de evaluación para certificado de avalúo
  propertyWriting: Home,        // Icono de casa para escritura de propiedad
  creditHistory: History,       // Icono de historial para historial de crédito
  bankAccountState: AccountBalance,  // Icono de banco para estado de cuenta
  workCertificate: Work,        // Icono de trabajo para certificado laboral
};
  ...
  const validations = {
    ...
    interestRate: (v) => {
      ...
      return rate >= 4 && rate <= 6 
        ? "" 
        : "La tasa debe estar entre 4% y 6%";
    },
    term: (v) => {
      const years = Number(v);
      return years >= 1 && years <= 20 
        ? "" 
        : "El plazo debe ser entre 1 y 20 años";
    },
  };
...
const RequestSecondHome = () => {
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      ...
      const response = await service.secondHouse(formDataToSend);
      ...
      <Paper elevation={3} sx={{ p: 4, borderRadius: 2 }}>
        <Box display="flex" justifyContent="space-between" alignItems="center">
          <Typography variant="h4" color="primary" gutterBottom>
            Solicitud {loanType === 'secondHome' ? 'Segunda Vivienda' : 'Primera Vivienda'}
          </Typography>
