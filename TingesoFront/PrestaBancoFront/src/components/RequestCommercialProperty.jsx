...
const RequestCommercialProperty = () => {
  ...
  const documentFields = [
    { name: 'businessFinancialStatement', label: 'Estados Financieros del Negocio', icon: <Upload /> },
    { name: 'proofIncome', label: 'Comprobante de Ingresos', icon: <Upload /> },
    { name: 'appraisalCertificate', label: 'Certificado de Avalúo', icon: <Upload /> },
    { name: 'businessPlan', label: 'Plan de Negocios', icon: <Upload /> },
    { name: 'bankAccountState', label: 'Estado de Cuenta Bancario', icon: <Upload /> },
    { name: 'workCertificate', label: 'Certificado Laboral', icon: <Upload /> },
  ];
  ...
  const handleSubmit = async (event) => {
    ...
    setLoading(true);
    try {
      await service.commercialProperty(formData);
      setSuccess(true);
      setTimeout(() => {
        navigate('/requests');
      }, 2000);
